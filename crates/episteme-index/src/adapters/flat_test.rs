...
fn dot_returns_highest_scoring_first() {
    ...
    let hits = FlatIndex.search(&store, &[1.0, 0.0], 2, None).unwrap();
...
fn l2_returns_lowest_distance_first() {
    ...
    let hits = FlatIndex.search(&store, &[0.0, 0.0], 3, None).unwrap();
...
fn dimension_mismatch_is_rejected() {
    ...
    let err = FlatIndex
        ...
        .unwrap_err();
    assert!(matches!(err, episteme_core::Error::DimMismatch { .. }));
}
...
fn absent_rows_are_skipped_not_scored() {
    ...
    let hits = FlatIndex.search(&store, &[1.0, 0.0], 10, None).unwrap();
...
fn filter_restricts_the_scan_and_still_fills_k() {
    ...
    let deny_first = |o: episteme_core::Ordinal| o.row() != 0;
