...
use telividb_core::{ScanTier, VectorStore};
use telividb_index::{
    FlatIndex, OverFetch, VectorIndex, adapters::MemoryStore, recall_at_k, two_tier_search,
};
use telividb_storage::format::quantize::{PqCodebook, PqParams};
use telividb_storage::{BinaryTier, Int8Tier, PqTier};
...
fn pq_needs_over_fetch_proportional_to_its_compression() {
    ...
    let refs: Vec<&[f32]> = (0..store.len())
        .filter_map(|r| store.get(telividb_core::Ordinal::from_row(r as u32)))
        .collect();
...
fn a_filtered_two_tier_search_matches_a_filtered_exhaustive_one() {
    let (store, queries) = corpus();
    let tier = Int8Tier::build(&store);
    let keep_even = |o: telividb_core::Ordinal| o.row().is_multiple_of(2);
