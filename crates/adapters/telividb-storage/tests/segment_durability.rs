...
use telividb_storage::format::Codec;
use telividb_storage::{SegmentWriter, field_dir, open_tier};
...
fn seal(
    ...
    let mut writer =
        SegmentWriter::create(&path, telividb_core::Fingerprint::unset()).expect("create");
    let s = store(rows, absent_every);
    writer
        .write_field_with_codec(name, &s, telividb_core::Fingerprint::unset(), codec)
        .expect("write field");
...
fn two_segments_in_one_directory_do_not_share_a_temp_path() {
    ...
    let mut a = SegmentWriter::create(&first, telividb_core::Fingerprint::unset()).unwrap();
    let mut b = SegmentWriter::create(&second, telividb_core::Fingerprint::unset()).unwrap();

    a.write_field("text", &s, telividb_core::Fingerprint::unset())
        .unwrap();
    b.write_field("text", &s, telividb_core::Fingerprint::unset())
        .unwrap();
