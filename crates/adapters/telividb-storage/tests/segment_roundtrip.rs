...
use telividb_index::{FlatIndex, VectorIndex};
use telividb_storage::{MutableBuffer, SegmentReader, SegmentWriter};
...
fn a_drifted_schema_is_refused_before_any_bytes_are_read() {
    ...
    assert!(matches!(err, telividb_storage::Error::SchemaDrift { .. }));
}
...
fn a_drifted_model_is_refused() {
    ...
    let err = reader
        ...
        .unwrap_err();
    assert!(matches!(err, telividb_storage::Error::ModelDrift { .. }));
}
