...
use telividb_core::{Metric, Ordinal, ScanTier, VectorStore};
use telividb_index::{FlatIndex, OverFetch, VectorIndex, recall_at_k, two_tier_search};
use telividb_storage::format::Codec;
use telividb_storage::{SegmentReader, SegmentWriter, segment::open_tier};
...
fn seal_and_reopen(
    ...
    let (store, _) = corpus();
    let schema = telividb_core::Fingerprint::of(b"schema-v1");
    let model = telividb_core::Fingerprint::of(b"model.gguf");
