...
use telividb_telemetry::{Telemetry, TelemetryConfig, catalogue, logger, metrics_names};
    ...
    RECORDING.get_or_init(|| {
        ...
        let bytes = runtime.block_on(async {
            let telemetry = Telemetry::install(TelemetryConfig {
                service: "telividb-pipeline-test".to_owned(),
...
fn a_disabled_pipeline_still_accepts_every_emission() {
    ...
    let meter = telividb_telemetry::Meter::disabled();
    for (name, kind, _) in catalogue::ALL {
        match kind {
            telividb_telemetry::MetricType::Counter => meter.counter(name, 1.0),
            telividb_telemetry::MetricType::Gauge => meter.gauge(name, 1.0),
            telividb_telemetry::MetricType::Histogram => meter.histogram(name, 1.0),
        }
    }
