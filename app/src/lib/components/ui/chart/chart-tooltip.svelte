<script lang="ts">
	...
	const formattedLabel = $derived.by(() => {
		...
		// This project compiles with `noUncheckedIndexedAccess`, so destructuring
		// the first series yields `T | undefined`. Upstream assumes it is present;
		// the guard above only checks `length`. Returning null is what the
		// component already does when there is nothing to label.
		if (!item) return null;
